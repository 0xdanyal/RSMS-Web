...
//           setShowPopup({ visible: true, message: 'Error fetching residents', isError: true });
//           setTimeout(() => setShowPopup({ visible: false, message: '', isError: false }), 3000);
//         } else {
...
//         const listingsResponse = await fetch(`http://localhost:5000/api/societies/${societyId}/listings`, {
//           headers: { 'Authorization': `Bearer ${token}` }
//         });
...
//   if (!societyId) {
//     console.error('No societyId available');
//     setShowPopup({ visible: true, message: 'No society ID available', isError: true });
...
// }
  ...
  const validateStaffForm = (isEdit = false) => {
    ...
    if (!staffForm.fullName) {
      ...
      errors.fullName = 'Name must be Ó2-100 characters, letters, spaces, hyphens, or apostrophes only';
    }
